...
public class BankAccountController {
    ...
    public ResponseEntity<String> createAccount(@RequestBody BankAccount new_account, HttpSession session) throws Exception {
		...
		return ResponseEntity.ok(create_status[1]); // idx 0是狀態 idx 1 是訊息回報
		}
    ...
    public List<BankAccount> getUserAccounts(HttpSession session) throws Exception {
		int user_id = (int) session.getAttribute("user_id");
